...
genes_counts <- read.csv("/gpfs/commons/groups/compbio/projects/ao_projects/ml_deconv_data_lineage/test/pseudobulk_raw.csv", header = T, check.names = F, row.names = 1)
...
singlespinal.sce <- subset(singlespinal.sce, subset = id %in% c("B15-CZI251N", "B4P1-CZI01N",  "B3-CZI11N",  "B16-CZI182N"))
#singlespinal.sce <- subset(singlespinal.sce, subset = newtype %in% c("EpithelialGL_harmonal", "Endothelial", "Stromal_EMT", "EpithelialPino"))
...
write.csv(results[["Est.prop.weighted"]],file="/gpfs/commons/groups/compbio/projects/ao_projects/ml_deconv/outputs/music_prop_pseudobulk_czi_vs_czi_single_cell_ref_estimated_heca_lineage_4_train_samples_lineage.csv")
